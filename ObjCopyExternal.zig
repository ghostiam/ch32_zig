...
pub fn create(
    ...
    const objcopy = owner.allocator.create(ObjCopyExternal) catch @panic("OOM");
    objcopy.* = ObjCopyExternal{
        .step = Step.init(.{
            .id = base_id,
...
pub fn getOutput(objcopy: *const ObjCopyExternal) std.Build.LazyPath {
    return .{ .generated = .{ .file = &objcopy.output_file } };
}
pub fn getOutputSeparatedDebug(objcopy: *const ObjCopyExternal) ?std.Build.LazyPath {
...
fn make(step: *Step, options: Step.MakeOptions) !void {
    ...
    man.hash.addBytes("external");
    man.hash.addBytes(objcopy_exe);
    ...
    try argv.appendSlice(&.{objcopy_exe});
